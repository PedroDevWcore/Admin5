...
router.post('/', authenticateToken, requireLevel(['super_admin', 'admin']), async (req, res) => {
  try {
    ...
    try {
      ...
      if (serverData[0]) {
        await wowzaConfigService.createWowzaConfig({
...
router.put('/:id', authenticateToken, requireLevel(['super_admin', 'admin']), async (req, res) => {
  try {
    ...
    try {
      ...
      if (serverData[0]) {
        await wowzaConfigService.updateWowzaConfig(
...
router.delete('/:id', authenticateToken, requireLevel(['super_admin', 'admin']), async (req, res) => {
  try {
    ...
    try {
      ...
      if (serverData[0]) {
        await wowzaConfigService.removeWowzaConfig(streaming[0].login, serverData[0].ip);
      }
    } catch (wowzaError) {
